                ...
                <div class="card-body">
                    @if (session('resent'))
                        <div class="alert alert-success" role="alert">
                            {{ __('На вашу электронную почту была отправлена новая ссылка для подтверждения') }}
                        </div>
                    @endif
                    ...
                    <form class="d-inline" method="POST" action="{{ route('verification.resend') }}">
                        @csrf
                        <button type="submit" class="btn btn-link p-0 m-0 align-baseline">{{ __('нажмите здесь, чтобы запросить еще одно письмо с ссылкой') }}</button>.
                    </form>
                </div>
            </div>
        </div>
